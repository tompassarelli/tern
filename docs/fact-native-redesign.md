...
`north time` and the rest retired in favor of north). The current operating
...
@X", not a string match. (`tag:north` → the north umbrella thread.)
...
- **Promotion prompts** (e.g. draft grew real structure → "commit it?"): not a
  ...
  event stream (`north watch`), surfaced for a human call.
...
as body + extract facts," not "title → emit a record." Reshape `north capture`
...
- **north engine** — `kernel` (drop state enum + tag; add relates_to + structural
...
**Stage 0 — Safety.** Tag `pre-fact-native` in north + north. Confirm
baseline round-trip + `north validate` + `los validate` all green.
...
**Stage 4 — Cutover.** Apply the migration to the live corpus; `north validate`
