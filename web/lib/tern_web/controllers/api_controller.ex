defmodule TernWeb.ApiController do
  ...
  def dag(conn, %{"focus" => focus}) when is_binary(focus) and focus != "",
    do: json(conn, Tern.Threads.focused(focus))
  ...
  def tell(conn, %{"graph" => g, "id" => id, "pred" => pred, "obj" => obj}) do
    te = Tern.Threads.resolve(at(id))
    r = if String.starts_with?(obj, "@"), do: Tern.Threads.resolve(obj), else: obj
    write_resp(conn, Tern.Fram.assert!(Tern.Fram.port_for(g), te, pred, r))
  end
  ...
  def capture(conn, %{"title" => title} = params) when is_binary(title) and title != "" do
    port = Tern.Fram.port_for(Map.get(params, "graph", "board"))
    id = "@" <> Tern.Id.uuid7()
    now = DateTime.utc_now() |> DateTime.to_iso8601()

    with {:ok, _} <- Tern.Fram.assert!(port, id, "title", title),
         {:ok, v} <- Tern.Fram.assert!(port, id, "created_at", now),
  ...
  def steer(conn, %{"handle" => h, "text" => text}) when is_binary(h) and is_binary(text) and text != "" do
    handle = h |> at() |> Tern.Threads.resolve() |> String.replace_prefix("@", "")
    write_resp(conn, Tern.Fram.assert!(Tern.Fram.agents_port(), "@session:" <> handle, "steer", text))
  end
  ...
  defp assert_optional_ref(port, id, pred, v) when is_binary(v),
    do: Tern.Fram.assert!(port, id, pred, Tern.Threads.resolve(at(v)))

  # wake's sibling /live WebSocket — upgrade + hand off to LiveFeed (PubSub-fed).
  def live(conn, _params) do
    conn
    |> WebSockAdapter.upgrade(TernWeb.LiveFeed, [], timeout: 120_000)
    |> halt()
  end
  ...
  <body>
    ...
    <script src="/js/tern-ui.js"></script>
  </body>
  </html>
  """
  ...
  <body>
    <div id="list"></div>
    <script src="/js/tern-list.js"></script>
  </body>
  </html>
  """
  ...
  def board_view(conn, _params),
    do: conn |> put_resp_content_type("text/html") |> send_resp(200, shell("board", "board", "/js/tern-board.js"))
  ...
  <body>
    ...
    <script src="/js/wake-mounts.js"></script>
    <script src="/js/tern-list.js"></script>
    <script src="/js/tern-board.js"></script>
    <script src="/js/tern-agents.js"></script>
    <script src="/js/tern-app.js"></script>
  </body>
  </html>
  """
