...
(function () {
  ...
  function selectAgent(handle) {
    ...
    selected = handle;
    window.tern.agentSelection = handle; // the frame "›" steers this agent
  ...
  async function render(root) {
    ...
    if (!selected || !find(selected)) {
      ...
      if (first) {
        ...
        window.tern.agentSelection = selected; // the frame "›" steers this agent
        if (chatEl) chatEl._stream = null;
        loadStream();
      }
    }
  ...
  window.tern.mountAgents = function ({ el: root }) {
    if (!root) return;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (root) window.tern.mountAgents({ el: root });
  });
})();
