// The north 2-panel client. LEFT = thread workbench (Graph | List | Board,
// toggled), RIGHT = agents. Each surface is an independent facts-native renderer
// (window.north.mount*); this just lays out the two frames, wires the view
...
(function () {
  ...
  function boot() {
    ...
    function show(key, opts) {
      ...
      mountInto(wb.content, window.north && window.north[v[2]], opts);
      current = key;
      paintToggle();
    }
    ...
    window.north.focusGraph = (id) => show("graph", { focus: id });
    ...
    mountInto(ag.content, window.north && window.north.mountAgents);
  }
