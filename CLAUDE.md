...
tern is the claim-native life/work app on the **fram** engine. This file is the
...
- **Dispatch**: `bun run ~/code/tern/sdk/src/dispatch.ts <thread-id>` — reads thread claims, derives posture (unplanned/atomic/composite), injects the right prompt + tool set, streams to the web client.
...
- **Work queue**: tern threads on **:7977** — `ready`/`next`/`leverage` to pick; claim a thread with `driver @agent`.
...
## Pointers
- tern thread `2026-06-23-132319` — CNF purity + tern-as-client architecture.
