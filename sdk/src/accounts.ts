...
const ROUTING_ECONOMICS_ENV = /^(?:AGENT_ROUTING_ASSESSMENT|NORTH_ROUTING_PIN_EVIDENCE)$/;

function deniedSubscriptionEnvironmentName(key: string): boolean {
  if (ROUTING_ECONOMICS_ENV.test(key) || PROVIDER_STATE_SELECTOR.test(key) || CLAUDE_CLOUD_TRANSPORT.test(key)
