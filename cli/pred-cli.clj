...
(def VOCAB
  [;; --- executable schema meta-predicates (the schema describes itself) ---
   ...
   ["worktree_allocation_thread" "single" "literal" "exact thread owner or explicit ad-hoc sentinel"]
   ["worktree_allocation_concern" "single" "literal" "exact concern owner or explicit unattributed sentinel"]
   ...
   ["native_command_activity_source" "single" "literal" "provider adapter source for observed native command activity"]
   ["native_command_activity_coverage" "single" "literal" "exact, partial, or unknown observed native command activity coverage"]
   ["native_north_binary_probe" "single" "literal" "passed, failed, or not_observed evidence of a native north binary probe"]
   ["native_command_total" "single" "literal" "total native commands observed for this run"]
   ["native_command_successful" "single" "literal" "successful native commands observed for this run"]
   ["native_command_failed" "single" "literal" "failed native commands observed for this run"]
   ["native_command_declined" "single" "literal" "declined native commands observed for this run"]
   ["native_command_truncated" "single" "literal" "truncated native commands observed for this run"]
   ["native_command_completion" "multi" "literal" "argument-free JSON command, output, and exit-status evidence for one native command completion"]
   ["completed" "single" "literal" "native command completion status enum literal captured by the fixed-predicate scanner alongside native_command_completion"]
   ["passed" "single" "literal" "native north binary probe status enum literal captured by the fixed-predicate scanner alongside native_north_binary_probe"]
   ...
   ["provider_join_key_version" "single" "literal" "privacy-bounded provider session/turn join-key domain version"]
   ["provider_join_coverage" "single" "literal" "provider join evidence coverage: exact, partial, or unknown"]
   ["provider_session_key" "single" "literal" "privacy-bounded SHA-256 key for one provider session"]
   ["provider_turn_key" "multi" "literal" "privacy-bounded SHA-256 key for one provider-owned turn"]
   ...
   ["staffing_catalog_sha256" "single" "literal" "SHA-256 of the staffing catalog used at admission (file mode only)"]
   ["orchestration_policy_pin_sha256" "single" "literal" "§3.2 verified three-way-equal selection-policy digest at admission (graph mode)"]
   ["orchestration_catalog_digest_sha256" "single" "literal" "§3.1(6) SHA-256 of the canonical catalog subgraph projection at admission (graph mode)"]
   ["orchestration_catalog_version" "single" "literal" "§3.1(6) @catalog:current pointer version admitted against (graph mode)"]
   ["orchestration_catalog_tx_version" "single" "literal" "§3.1(6) coordinator global tx watermark at catalog projection (graph mode)"]
