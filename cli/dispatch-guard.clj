...
;; Returns exit code: 0=GREEN (reuse), 1=YELLOW (reuse with caution),
;; 2=REPLACE (fresh managed lane), 3=PINNED (always reuse).
;;
;; usage:
...
(defn replacement-guidance [uuid reason]
  (str "-> REPLACE: do not reuse agent " uuid " (" reason "). "
       "The coordinator must delegate a fresh managed lane with an explicit "
       "context brief; North has no provider-neutral in-place context "
...
(let [[port-s target] *command-line-args*
      ...
      bucket (cond needs-rotation :replace pinned :pinned (< score 0.3) :green (< score 0.7) :yellow :else :red)]
  ...
  (case bucket
    ...
    :red    (do (println (replacement-guidance uuid "stale context"))
                (System/exit 2))))
