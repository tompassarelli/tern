...
;;
;; usage:
;;   bb tern-map.clj <port> map    <role-template> <N> <task> [K]   — register batch + fan out N workers
;;   bb tern-map.clj <port> done   <batch-id> <worker> <payload>    — a worker reports DONE (carries payload)
;;   bb tern-map.clj <port> status <batch-id>                       — barrier state + aggregated payloads
;;   bb tern-map.clj <port> barrier <batch-id>                      — just the derived fired? + done set
;;   bb tern-map.clj <port> list                                    — known batches
...
(def distinct-of tern.coord/distinct-of)   ; count-distinct quorum, set form
...
(let [[port verb & args] *command-line-args*
      ...
      scratch (str home "/code/tern/cli")]
  (case verb
        ...
        (let [slug (str tmpl "-" id "-" i)]
          ...
          (when spawn?
            ...
            (let [full-prompt (str task "\n\n[batch " e " worker " slug "]\n"
                                   ...
                                   "  bb " scratch "/tern-map.clj " port " done " id " " slug " \"<your result>\""
                                   (when has-schema
              ...
              (sh "/run/current-system/sw/bin/bun" "run" (str home "/code/tern/sdk/src/spawn.ts") full-prompt
    ...
    (let [[id worker payload] args
          ...
          vr (tern.schema-validate/validate-json payload schema)]
      (when-not (:valid vr)
        ...
        (println (str "  ↳ DONE not recorded; barrier unchanged. Fix the payload to satisfy the schema and "
                      "re-run:\n     bb <tern-map.clj> " port " done " id " " worker " \"<conforming-json>\""))
        (System/exit 3))
