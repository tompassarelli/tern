...
(def rf      tern.coord/resolved)
(def rmany   tern.coord/many)
...
(def agg-rows       tern.coord/agg-rows)
(def sum-rows       tern.coord/sum-rows)
(def count-distinct tern.coord/count-distinct)
...
;; any concurrency ceiling = count(live drivers) vs TERN_SWARM_MAX.
(def driver-max (Integer/parseInt (or (System/getenv "TERN_SWARM_MAX") "64")))
